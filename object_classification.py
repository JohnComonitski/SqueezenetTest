...
#Loads in a fill folder of images
def loadImageSet(folder):
    fileNames = []
    images = []
...
def main():
    ...
    # Reads images and saves their file names
    testImages, testNames = loadImageSet(args.input[0])
    #Reshapes images, modified from original to reshape all images
    ...
    for j in range(len(testImages)):
        for i in range(n):
            ...
            imagesTemp = np.ndarray(shape=(n, c, h, w))
            imagesTemp[i] = image
            testImages[j] = imagesTemp
    log.info("Batch size is {}".format(len(testImages)))
    ...
    #Preps output file
    outFile = open("output-" +netName+ ".csv","w+")
    outFile.close()
        ...
        for i, probs in enumerate(res):
            ...
            for id in top_ind:
                ...
                print("{}{}{}{}{:.7f}".format(' ' * space_num_before, det_label,
                                              ...
                                              probs[id]))
                #Some Squeezenet labels include commas, here we are removing them to not ruin the csv
                if(det_label.find(',') == -1):
                    output += ",{},{:.7f}".format(det_label,probs[id])
                else:
                    output += ",{},{:.7f}".format(det_label[0:det_label.find(',')],probs[id])
            print("\n")
            output += "," + testNames[j][0:-5]
            #Checks to see if guess was correct
            if(testNames[j][0:-5] == labels_map[top_ind[0]]):
                output += ",1"
            else:
                output += ",0"
        #outputs CSV
